...
func (s *Session) listen(wsConn *websocket.Conn, listening <-chan interface{}) {
	...
	for {
		...
		log.Printf("[discordgo raw] type=%d len=%d msg=%.200s", messageType, len(message), string(message))
		if err != nil {
...
func (s *Session) identify() error {
	...
	if err == nil {
		log.Printf("[discordgo identify] %s", string(data))
	} else {
		s.log(LogError, "error marshaling identify packet for logging: %s", err)
	}
	s.wsMutex.Lock()
	err = s.wsConn.WriteJSON(op) // This uses the 'op' variable defined above
	s.wsMutex.Unlock()
